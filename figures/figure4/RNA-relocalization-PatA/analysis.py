...
@app.cell
def _(mcolors, np):
    def merge_images_with_cmaps(
        img1, img2, cmap1, cmap2, vmin1=None, vmax1=None, vmin2=None, vmax2=None
    ):
        """
...
def _(
    ...
    def process_sample(
        ...
        **kwargs,
        ...
        condition_translate = {"ctrl": "DMSO Control", "exp": "PatA Treated"}
...
def _(Path):
    experiment_root = Path(
        "/Users/julian/Documents/General Science/Programming/py/general_analysis/20251130_PatA_RNA_loc/data/RNAscope_tdrd7_nanos/collected_data"
    )
...
@app.cell
def _(df):
    df.groupby(by="Condition").agg("count")
    return
...
def _(df, plt, sns):
    ...
    fig_paper_new_style, ax_paper_newstyle = plt.subplots(1, 1, figsize=(1.64, 3))
    ...
    ax_paper_newstyle.set_title("mRNA relocalization - PatA", fontweight="bold")
    ...
    sns.despine()
    # plt.show()
    fig_paper_new_style.savefig(
        "proto/output/paper_figure_newstyle.pdf", transparent=True
    )
    return
...
def _(df, pd):
    ...
    df_significance.to_csv("proto/output/statistical_testing_results.csv", index=False)
    return


@app.cell
def _(np):
    def get_representative_ids(
        the_df, feature, low_percentile, high_percentile, conditions="all"
    ):
        """
        ...
        for condi in the_df["Condition"].unique():
            ...
            for rna_tpe in ["nanos", "tdrd7"]:
                val_low = np.percentile(
                    cond_df.query(f"RNAType == '{rna_tpe}'")[feature], low_percentile
                )
                val_high = np.percentile(
                    cond_df.query(f"RNAType == '{rna_tpe}'")[feature], high_percentile
                )
                rna_sub_df = cond_df.query(f"RNAType == '{rna_tpe}'")

                mask = (rna_sub_df[feature] > val_low) & (
                    rna_sub_df[feature] < val_high
                )
                ...
                if not representative_ids:
                    ...
                    rep_list = [
                        rep
                        for rep in representative_dfs["UID"].tolist()
                        if rep in representative_ids
                    ]
