...
def process_sample(sample_dir, ax=None, percentile_vasa=99.0, percentile_rna=99.0, save_images=False, **kwargs):
    ...
    seg_granules = measure.label(seg_granules)#.astype(bool) # Ensure granule segmentation is binary
...
list_sample_dirs = sorted([d for d in root.iterdir() if d.is_dir() and not d.name.startswith(".")])
...
for i, sample_directory in enumerate(list_sample_dirs):
    ...
    result_collector.extend(process_sample(sample_directory, ax=axe, percentile_vasa=99.9, percentile_rna=99.4, distance_in=3, distance_out=5, save_images=True))
